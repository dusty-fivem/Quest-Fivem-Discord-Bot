    ...
    if (interaction.customId == "open-ticket") {
      ...
      interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
        ...
        const embed = new client.discord.MessageEmbed()
          ...
          .setAuthor({name: `${interaction.user.username}'s Ticket`, iconURL: (config.SERVER_LOGO)})
        ...
        if (i.customId == 'confirm-close') {
          ...
          chan.edit({
            ...
            .then(async () => {
              const embed = new client.discord.MessageEmbed()
                .setColor(config.EMBED_COLOR)
                .setAuthor({name: 'Ticket', iconURL: (config.SERVER_LOGO)})
    ...
    if (interaction.customId == "delete-ticket") {
      ...
      chan.messages.fetch().then(async (messages) => {
        ...
        var result = await paste.uploadContent(a, {uploadFormat: 'markdown'})
            const embed = new client.discord.MessageEmbed()
              .setAuthor({name: 'Ticket Logs', iconURL: (config.SERVER_LOGO)})
            ...
            setTimeout(() => {
              if ((guild).channels.cache.find(c => c.id == (chan))) {
              return chan.delete();}
            }, 5000);
          });
    };
